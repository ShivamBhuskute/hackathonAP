...
export default function UniversityAdminRegistration({ navigation }) { // Added navigation prop here
  ...
  const handleRegistration = () => {
    if (!name || !email || !password || !confirmPassword || !contactNumber || !universityName || !verificationId) {
    ...
    Alert.alert('Success', 'Registration completed successfully.', [
      {
        text: 'OK',
        onPress: () => navigation.navigate('UniversityDashboard'), // Use navigation here
      },
    ]);
  };
  ...
  useEffect(() => {
    ...
    Animated.loop(
      Animated.stagger(1000, bubblePositions.map((position, index) =>
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Animated.Text style={[styles.title, { opacity: fadeAnim }]}>University Admin Registration</Animated.Text>
